-- =============================================================================
-- 091 — resolved_transactions sources balance_after from header-account table
-- =============================================================================
--
-- ADR-0034 part 3. balance_after was l.balance_after (txn_legs column,
-- maintained by the leg-walk trigger). With mig 089/090 it now lives in
-- txn_header_account_balances keyed by (header_id, account_id).
--
-- Same column position and shape (NUMERIC nullable) — every consumer of
-- the view (ResolvedTransactionView EF entity, RegisterRepository, SPA
-- DTOs) keeps working without modification.
--
-- security_invoker = true re-asserted per the migration 057 / 077 / 079
...
SELECT l.id,
    ...
    -- ADR-0034: balance_after now keyed by (header, account), not by leg.
    -- All cash legs of a header on this account share the same value, so
    -- the LEFT JOIN below returns a single row per (header, account).
    thab.balance_after,
    h.origin,
    h.is_pending,
    h.is_merged_into,

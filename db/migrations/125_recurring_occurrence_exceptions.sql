-- =============================================================================
-- 125 — recurring_occurrence_exceptions: suppress one (series, date) slot
--        from the upcoming expansion (ADR-0047 D6 "skip the next occurrence")
-- =============================================================================
--
-- The C# RecurrenceExpander generates upcoming slots from
-- recurring_transactions.rrule. A row here marks ONE expanded slot suppressed
-- (the user skipped it): the slot never reappears in /reminders/upcoming and
-- RemindersRepository.FireAsync refuses to materialize it. A skip writes NO
-- header and never mutates committed cash (committed occurrences are immutable
-- live headers, ADR-0047 D6) — skip and fire are mutually exclusive per slot,
-- enforced in the repository.
--
-- Identity:
--   * UNIQUE (recurring_transaction_id, occurrence_date) — one suppression per
--     slot. This btree IS the expander's anti-join lookup index; no extra
--     index needed.
--
-- FKs:
--   * (recurring_transaction_id, ledger_id) -> recurring_transactions(id,
--     ledger_id) ON DELETE CASCADE. Composite, ledger-scoped (uses
--     uq_recurring_transactions_id_ledger, mig 124). CASCADE — a suppression is
--     pure series-local metadata carrying no cash; it is meaningless once the
--     series is gone. (Contrast: txn_headers.recurring_transaction_id is SET
--     NULL because a committed occurrence carries real cash and must survive a
--     series delete.) No DEFERRABLE: this table only POINTS AT
--     recurring_transactions (no mutual cycle), so an immediate FK is correct.
--   * ledger_id -> ledgers(id) ON DELETE CASCADE.
--   * created_by_user_id -> users(id) ON DELETE SET NULL (audit outlives user).
--
-- Identity column: API-supplied UUID (no DEFAULT) — the repository sets
-- Id = Guid.NewGuid(), matching mig 075 / the house insert pattern.
--
-- RLS: same flattened per-ledger policy as migrations 071/072/075.
-- =============================================================================
...
CREATE TABLE recurring_occurrence_exceptions (
    ...
    CONSTRAINT fk_recurring_occurrence_exceptions_ledger
        FOREIGN KEY (ledger_id) REFERENCES ledgers(id) ON DELETE CASCADE,
    CONSTRAINT fk_recurring_occurrence_exceptions_series
        FOREIGN KEY (recurring_transaction_id, ledger_id)
        REFERENCES recurring_transactions(id, ledger_id) ON DELETE CASCADE,
    CONSTRAINT fk_recurring_occurrence_exceptions_user
        FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON DELETE SET NULL,

    CONSTRAINT uq_recurring_occurrence_exceptions_slot
        UNIQUE (recurring_transaction_id, occurrence_date)
);

COMMENT ON TABLE recurring_occurrence_exceptions IS
    'ADR-0047 D6: one row suppresses a single (series, occurrence_date) slot '
    'from the upcoming expansion. Skip and fire are mutually exclusive per slot '
    '(repository-enforced). No cash, no header.';

-- -----------------------------------------------------------------------------
-- RLS — same per-ledger flattened policy as migrations 071/072/075.
-- -----------------------------------------------------------------------------

ALTER TABLE recurring_occurrence_exceptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE recurring_occurrence_exceptions FORCE  ROW LEVEL SECURITY;

DROP POLICY IF EXISTS recurring_occurrence_exceptions_per_user ON recurring_occurrence_exceptions;
CREATE POLICY recurring_occurrence_exceptions_per_user ON recurring_occurrence_exceptions
    FOR ALL
    TO coffer_app
    USING (ledger_id IN (
...
-- coffer_service bypasses RLS (BYPASSRLS attribute); coffer_app is the
-- per-request role gated by the policy.
GRANT SELECT, INSERT, UPDATE, DELETE ON recurring_occurrence_exceptions TO coffer_app;
GRANT ALL ON recurring_occurrence_exceptions TO coffer_service;

-- =============================================================================
-- recurring_transactions: end_date >= start_date invariant (ADR-0047)
-- =============================================================================
--
-- The manual-authoring edit surface (this slice) can apply start/end one at a
-- time, which without a guard could persist an inverted [start, end] range -
-- a silently dormant series (its expansion window is empty). The DB owns this
-- invariant (constraints-over-workarounds); the edit endpoints add a clean 422
-- on top (defense in depth). Scrub-then-constrain in one migration: a
-- nonsensical end-before-start is repaired to NULL ("no end") - a no-op for
-- all legitimate data (imported reminders always have end >= start).
-- =============================================================================
UPDATE recurring_transactions SET end_date = NULL
 WHERE end_date IS NOT NULL AND end_date < start_date;

ALTER TABLE recurring_transactions
    ADD CONSTRAINT ck_recurring_transactions_end_after_start
        CHECK (end_date IS NULL OR end_date >= start_date);

-- =============================================================================
-- recurring_transactions.source_account_id - display/query pointer (ADR-0047)
-- =============================================================================
--
-- mig 124 moved the transaction SHAPE onto the template legs and dropped the
-- denormalized source_account_id. The agenda/calendar (Moneydance parity) shows
-- each series' AMOUNT = the cash net on its ORIGINATING account, which is NOT
-- recoverable from the legs for a symmetric single posting (the two legs are
-- equal-and-opposite, so any leg-only heuristic nets to zero). So we
-- re-introduce source_account_id - NOT as transaction shape (that stays on the
-- legs), but as a pointer to the originating account; the amount is summed from
-- the template legs ON this account. Nullable: NULL on a custom reminder with
-- no single source and on pre-125 rows until a re-import / edit sets it (amount
-- then reads 0, never wrong).
--
-- Composite FK (source_account_id, ledger_id) -> accounts(id, ledger_id) - the
-- mig-049/072 ledger-coherence pattern (accounts carries the (id, ledger_id)
-- unique). ON DELETE RESTRICT: the template legs already pin this account, so
-- this adds no new delete restriction.
-- =============================================================================
ALTER TABLE recurring_transactions
...
-- =============================================================================
-- Snapshot coverage (the mig 111 -> 112 lesson, ADR-0037)
-- =============================================================================
--
-- The snapshot round-trip is driven by these two SQL functions (the C#
-- LedgerSnapshotPayload.InScopeTables list is now documentation/test-reference
-- only). to_jsonb / jsonb_populate_recordset do NOT auto-discover a new table,
-- so without enumerating it here a restore would silently drop every
-- suppression — the exact failure mode mig 112 fixed for recurring_transactions
-- + security_splits. Both functions are reproduced verbatim from mig 112 with
-- ONLY the recurring_occurrence_exceptions line/block added (no logic change).
--
-- FK ordering: recurring_occurrence_exceptions references recurring_transactions
-- (and ledgers). So on DELETE it must go BEFORE recurring_transactions
-- (reverse-FK); on INSERT it must go AFTER recurring_transactions (forward-FK).
-- =============================================================================
...
BEGIN
    ...
    -- recurring_occurrence_exceptions references recurring_transactions (now
    -- present) — insert after it (forward-FK).

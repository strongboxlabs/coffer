-- =============================================================================
-- 100 — resolved_transactions exposes header_account_net_amount
-- =============================================================================
--
-- Adds `header_account_net_amount` projection alongside the existing
-- `balance_after` column — both sourced from
-- txn_header_account_balances (mig 089/091/098). Single fetch for both
-- the running total and its per-step delta.
--
-- Per-leg `amount` stays as-is (the split editor + bank-shape consumers
-- still need leg granularity). Multi-leg / multi-posting headers on
-- one account now have the same `header_account_net_amount` on every
-- leg of (header, account) — the SPA can read it once per entry
-- instead of summing legs.
--
-- security_invoker = true re-asserted per the migration 057 / 077 / 091
-- pattern (CREATE OR REPLACE VIEW preserves data but NOT view-level
-- settings).
-- =============================================================================
...
SELECT l.id,
    ...
    -- Same value on every leg of (account, header); SPA reads it
    -- once per entry instead of summing legs at render time.

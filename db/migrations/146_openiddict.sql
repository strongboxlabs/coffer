-- =============================================================================
-- 146 — OpenIddict OAuth 2.1 authorization-server tables (ADR-0063 §D2)
-- =============================================================================
--
-- The MCP server's native one-click connector flow (claude.ai / Claude Desktop)
-- needs an OAuth 2.1 AS: discovery, dynamic client registration, authorization-
-- code + PKCE, refresh, and revocable reference access tokens. OpenIddict
-- provides that; these are its Entity Framework Core stores.
--
-- This project uses DbUp (not EF migrations), so the schema is hand-authored
-- here — transcribed VERBATIM from OpenIddict 7.5.0's EF Core model
-- (DbContext.Database.GenerateCreateScript), quoted PascalCase identifiers and
-- all, so EF's model maps cleanly. Do not rename columns: the ORM expects these
-- exact names.
--
-- Access: OpenIddict's stores run inside the request pipeline on the AppDbContext
-- (coffer_app at request time), so coffer_app gets full CRUD. No RLS — these are
-- global authorization-server state (clients, grants, tokens), not per-user
-- ledger rows; OpenIddict enforces its own access rules and the Subject column
-- carries the owning user. The whole surface is gated off unless MCP is enabled
-- (Api:Mcp:Enabled), but the tables exist regardless so DbUp stays linear.
-- =============================================================================
...
-- coffer_app (request-time role) drives OpenIddict's stores during /authorize,
-- /token, DCR, and /mcp token validation. coffer_service owns everything (admin
-- + maintenance paths).

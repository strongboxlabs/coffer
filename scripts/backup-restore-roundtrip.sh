...
su_psql() { docker exec -i "$PG_CONTAINER" psql -v ON_ERROR_STOP=1 -q -U "$SUPERUSER" -d "$1"; }
...
CREATE TABLE parent (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), name text);
...
# After the wipe: app objects gone, extensions preserved.
...
echo "   OK: schema emptied of app tables; pgcrypto/pg_trgm preserved."
